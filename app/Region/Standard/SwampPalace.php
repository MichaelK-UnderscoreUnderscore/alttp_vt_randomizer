    ...
    public function initalize()
    {
        ...
        $mire = function ($locations, $items) {
            return $this->world->config('canOneFrameClipUW', false)
                && (($locations->itemInLocations(Item::get('BigKeyD6', $this->world), ["Misery Mire - Compass Chest", "Misery Mire - Big Key Chest",]) && $items->has('KeyD6', 2))
        ...
        $this->locations["Swamp Palace - Big Chest"]->setRequirements(function ($locations, $items) use ($mire, $hera) {
            return ($items->has('KeyD2')
                    || $mire($locations, $items))
                && ($items->has('Hammer') || $mire($locations, $items) || $hera($locations, $items))
                && ($items->has('BigKeyD2')
                    || ($mire($locations, $items)
                        && $items->has('BigKeyD6'))
